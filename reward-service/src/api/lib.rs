...
use crate::{
    api::{
        ...
        error::ErrorResponse,
        middleware,
        resources::{health, swagger, reward},
    },
    domain::{reward::repository::RewardRepository, health::repository::HealthRepository},
    repository::{reward::PgRewardRepository, health::PgHealthRepository, postgres},
};

pub struct AppState {
    pub health_repository: Arc<dyn HealthRepository>,
    pub reward_repository: Arc<dyn RewardRepository>,
}
...
pub async fn run(pg_pool: Arc<Pool>, redis_client: Arc<Client>) -> Result<(), Box<dyn Error>> {
    ...
    let repositories = Data::new(AppState {
        ...
        reward_repository: Arc::new(PgRewardRepository::new(pg_pool.clone())),
    });
