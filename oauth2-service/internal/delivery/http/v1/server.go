...
func (s *Server) redirectError(w http.ResponseWriter, req *config.AuthorizeRequest, err error) error {
	if req == nil {
		return err
	}
...
func (s *Server) redirect(w http.ResponseWriter, req *config.AuthorizeRequest, data map[string]interface{}) error {
...
func (s *Server) GetRedirectURI(req *config.AuthorizeRequest, data map[string]interface{}) (string, error) {
...
func (s *Server) ValidationAuthorizeRequest(r *http.Request) (*config.AuthorizeRequest, error) {
	...
	if resType.String() == "" {
		return nil, config.ErrUnsupportedResponseType
	} else if allowed := s.CheckResponseType(resType); !allowed {
		return nil, config.ErrUnauthorizedClient
	}

	cc := r.FormValue("code_challenge")
	if cc == "" && s.Config.ForcePKCE {
		return nil, config.ErrCodeChallengeRquired
	}
	if cc != "" && (len(cc) < 43 || len(cc) > 128) {
		return nil, config.ErrInvalidCodeChallengeLen
	}
	...
	if ccm != "" && !s.CheckCodeChallengeMethod(ccm) {
		return nil, config.ErrUnsupportedCodeChallengeMethod
	}
...
func (s *Server) GetAuthorizeToken(ctx context.Context, req *config.AuthorizeRequest) (token.TokenInfo, error) {
	...
	tgr := &token.TokenGenerateRequest{
	...
	if fn := s.ClientScopeHandler; fn != nil {
		...
		if err != nil {
			...
			return nil, config.ErrInvalidScope
		}
	}
...
func (s *Server) GetAuthorizeData(rt config.ResponseType, ti token.TokenInfo) map[string]interface{} {
	if rt == config.Code {
...
func (s *Server) ValidationTokenRequest(r *http.Request) (config.GrantType, *token.TokenGenerateRequest, error) {
	...
	if gt.String() == "" {
		return "", nil, config.ErrUnsupportedGrantType
	}
	...
	case config.PasswordCredentials:
		...
		if err != nil {
			...
			return "", nil, config.ErrInvalidGrant
		}
		tgr.UserID = userID
...
func (s *Server) GetAccessToken(ctx context.Context, gt config.GrantType, tgr *token.TokenGenerateRequest) (token.TokenInfo,
	error) {
	...
	case config.AuthorizationCode:
		...
		if err != nil {
			switch err {
			case config.ErrInvalidAuthorizeCode, config.ErrInvalidCodeChallenge, config.ErrMissingCodeChallenge:
				return nil, config.ErrInvalidGrant
			case config.ErrInvalidClient:
	...
	case config.Refreshing:
		...
		if scopeFn := s.RefreshingScopeHandler; tgr.Scope != "" && scopeFn != nil {
			...
			if err != nil {
				if err == config.ErrInvalidRefreshToken || err == config.ErrExpiredRefreshToken {
					return nil, config.ErrInvalidGrant
				}
				return nil, err
			}
...
func (s *Server) GetTokenData(ti token.TokenInfo) map[string]interface{} {
...
func (s *Server) GetErrorData(err error) (map[string]interface{}, int, http.Header) {
	var re config.Response
	if v, ok := config.Descriptions[err]; ok {
		re.Error = err
		re.Description = v
		re.StatusCode = config.StatusCodes[err]
	} else {
		...
		if re.Error == nil {
			re.Error = config.ErrServerError
			re.Description = config.Descriptions[config.ErrServerError]
			re.StatusCode = config.StatusCodes[config.ErrServerError]
		}
	}
...
func (s *Server) ValidationBearerToken(r *http.Request) (token.TokenInfo, error) {
	ctx := r.Context()
